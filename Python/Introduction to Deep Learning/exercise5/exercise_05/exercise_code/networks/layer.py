...
class Relu:
    ...
    def backward(self, dout, cache):
        ...
        out = np.maximum(cache, 0)
        out_logi = np.ones(np.shape(out)) * (out>0)
        dx = out_logi * dout
        # pass
...
def affine_forward(x, w, b):
    ...
    x_resh = x.reshape((N, int(x.size/N))) # int(x.size/N) is D, so x_resh is of shape (N, D)
    out = np.dot(x_resh,w) + b.transpose()
    # pass
...
def affine_backward(dout, cache):
    ...
    dx_ToReshap = np.dot(dout, w.transpose())
    dx = dx_ToReshap.reshape(x.shape)

    x_resh = x.reshape((x.shape[0], int(x.size/x.shape[0])))
    dw = np.dot(x_resh.transpose(), dout)

    db = np.sum(dout, axis=0).transpose()
    # pass
