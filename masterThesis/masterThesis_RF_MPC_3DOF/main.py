...
for ii in range(MAX_ITER):
    ...
    # print([ "{:.4f}".format(x) for x in zval ])
    
    Ut = Ut + zval[0:12] # desired GRF of four legs in 3 coordinates
    # print("Xt=", Xt)
    # print("Xd=", Xd[:, 0])
    print("Ut=", Ut)

    ctrlData = robot.computeTorque(FSM, Xt, Ut, Xd) # compute torque based on desired GRF and swing leg trajectory
    u_real = robot.step(ctrlData)
    Xt = robot.get_state()
    print("Real Ctrldata:", robot.sim.data.actuator_force)
    # print('ctrldata=', ctrldata)
    
    # Plotting list update
    # PlotList.update(ii/MAX_ITER, Xt, Xd[:, 0], Ut, u_real)
    
tsimend = time.time()
print('The total simulation time is', tsimend - tsimstart, 'sec.')

# PlotList.result_plot()