...
class mujoco_robot:
    ...
    def set_init_state(self):
        ...
        self.footPos_3d = np.zeros((12, )) # foot position in 3 dimension
        for i in range(4):
    ...
    def idle_motion(self):
        ...
        for loop in range(1000):
            ...
            for i in range(4):
                ...
                footPos_des = np.array([0, self.z0 + 0.010]) # Plus 0.05 of z-coordinate to offset the gravity influence of VMC
                # print(footPos_des)
    ...
    def get_state(self):
        ## Related to Main Body COM
        # COM Angle Position Measurement
        ...
        # print("touch sensor:", self.sim.data.sensordata[23:27])
    ...
    def swing_leg_control(self, leg_ID, Xt, Xd, printResult):
        ...
        # and no longer follow the swing leg trajectory to prevent the foot from bouncing，
        ...
        if self.IsContact_Count[leg_ID - 1] >= 1:
            ...
            Kp = 0.01 * Kp
            Kd = 0.01 * Kd
            # footPos_des = footPos_Pre
        # else:
        footPos_des_w = np.array([Xd[18+(leg_ID-1)*3, 0], Xd[18+(leg_ID-1)*3+1, 0], Xd[18+(leg_ID-1)*3+2, 0]]) # Xd computed the foot Position under global frame
        COMPos_w = np.array(Xd[0:3, 0]) # Desired COM position under global frame
        COM2foot_w = footPos_des_w - COMPos_w
        R = np.reshape(Xd[6:15, 0], (3, 3), order='F')
        COM2foot_b = R.T @ COM2foot_w
        COM2hip_b = self.hipPos[:, leg_ID- 1]
        hip2foot_b = COM2foot_b - COM2hip_b
        hip2foot_l = np.array([-hip2foot_b[0], -hip2foot_b[2]]) # from body frame to leg frame
