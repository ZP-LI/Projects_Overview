...
def get_params(gait):
    p = {}
    p['predHorizon'] = 6
    p['simTimeStep'] = 1/2000
    p['Tmpc'] = 4/1000 # MPC prediction step time
    p['gait'] = gait
    p['Umax'] = 100
    p['decayRate'] = 1
    p['freq'] = 30
    ...
    if gait == 1:           # 1 - bound
        ...
        p['Tmpc'] = 2/1000
        p['decayRate'] = 1
        p['R'] = np.diag(np.squeeze(np.tile(np.array([[0.1, 0.1, 0.1]]).T, (4, 1)).T))
    ...
    ## Physical Parameters
    p['mass'] = 5.5
    p['J'] = np.diag([4.695e-03, 7.632e-02, 7.873e-02])
    ...
    p['J_singleLeg'] = np.array([1.045e-04, 1.045e-04, 2.75e-06]) # Up/Down Leg Inertia
    p['m_UpLeg'] = 0.055             # Up Leg Mass
    p['m_DownLeg'] = 0.055           # Down Leg Mass

    ## Swing phase
    p['Kp_sw'] = np.array([1600, 1600])   # Kp for swing phase # 1 for timestep 0.01
    p['Kd_sw'] = np.array([5, 5])       # Kd for swing phase # 0.01 for timestep 0.01
    p['Kp_init'] = np.array([1000, 500])   # Kp for init stance phase
    p['Kd_init'] = np.array([40, 40])       # Kd for init stance phase
