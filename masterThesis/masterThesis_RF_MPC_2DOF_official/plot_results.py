...
#   - Foot position (specially for swing leg control)
...
class plot_results:
    ...
    def result_plot(self):
        ...
        for i in range(3):
            for j in range(4):
                l1, = axs1[i,j].plot(self.tList, self.UtList[i+j*3], color="green", linewidth=2, label='desired')
                l2, = axs1[i,j].plot(self.tList, self.UList[i+j*3], color="blue", alpha=0.5, label='real')
                axs1[i,j].grid()
                if i == 0:
        ...
        for i in range(3):
            for j in range(4):
                l1, = axs2[i,j].plot(self.tList, self.XdList[i+j*3], color="green", linewidth=2, label='desired')
                l2, = axs2[i,j].plot(self.tList, self.XtList[i+j*3], color="blue", alpha=0.7, label='real')
                axs2[i,j].grid()
                if i == 0:
        ...
        for i in range(3):
            for j in range(4):
                l1, = axs3[i,j].plot(self.tList, self.FPdList[i+j*3], color="green", linewidth=2, label='desired')
                l2, = axs3[i,j].plot(self.tList, self.FPtList[i+j*3], color="blue", alpha=0.7, label='real')
                axs3[i,j].grid()
                if i == 0:
