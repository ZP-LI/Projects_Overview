...
def walking_test(mouse):
    ...
    trajectory_rr = [n_r,n_r,n_r]
    trajectory_rf = [n_f,n_f,n_f]
...
def leg_movement(mouse, max_vel, trajectory_r, max_step_r, trajectory_f, max_step_f):
    ...
    while(count < 2000):
        ...
        max_vel_s = np.array([0,0,0,rospy.get_param("/vel_rx"),0])
...
def leg_movement2(mouse, max_vel, trajectory_r, max_step_r, trajectory_f, max_step_f, trajectory_rf, trajectory_rr):
    ...
    while(True):
        ...
        csv_fl = sim.data.sensordata[:]
        csv_fr = sim.data.sensordata[2:]
        csv_rl = sim.data.sensordata[4:]
        csv_rr = sim.data.sensordata[6:]
...
def full_body_motion():
    ...
    while(True):
        ...
        q_rest = np.array([0,0,0,0])
        q_vals = np.concatenate((q_vals, q_rest))
