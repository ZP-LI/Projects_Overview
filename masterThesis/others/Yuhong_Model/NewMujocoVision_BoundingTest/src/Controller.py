...
class MouseController(object):
	...
	def __init__(self, fre, time_step, spine_angle):
		...
		# self.period = 3/2
		# self.fre_cyc = fre
		# self.SteNum = 32/2/PI# int(1/(time_step*self.fre_cyc))	#32 # Devide 2*PI to multiple steps
		...
		self.spinePhase = self.phaseDiff[3]
		# --------------------------------------------------------------------- #
		# self.phaseDiff = [PI+0.3*PI, PI+0.1*PI, 0, 0.1*PI]			# Gallop
		# self.period = 1.9/2
		# self.fre_cyc = fre
		# self.SteNum = int(1/(time_step*self.fre_cyc)) # 0.5步/秒，0.002秒/timestep -> SteNum = 几个tiemstep/步
		# print("----> ", self.SteNum)
		...
		self.trgYList = [[],[],[],[]]
		# --------------------------------------------------------------------- #
		self.errorXList = [[0],[0],[0],[0]]
		self.errorYList = [[0],[0],[0],[0]]
	...
	def getLegCtrl(self, leg_M, curRad, leg_ID):
		...
		trg_x += self.errorXList[leg_ID][-1]*0
		trg_y += self.errorYList[leg_ID][-1]*0
