...
class SimModel(object):
	...
	def __init__(self, modelPath, ):
		...
		self.actuatorForce_Max = np.zeros(13)
		self.actuatorForce_Min = np.zeros(13)
		self.actuatorForces = [[]]
	...
	def runStep(self, ctrlData, cur_time_step):
		...
		self.actuatorForce_Max = np.maximum(self.actuatorForce_Max, np.round_(self.sim.data.actuator_force, decimals=2))
		self.actuatorForce_Min = np.minimum(self.actuatorForce_Min, np.round_(self.sim.data.actuator_force, decimals=2))
		self.actuatorForces.append(np.round_(self.sim.data.actuator_force, decimals=2))
	...
	def ActuatorForce(self):
		print("Max Actuator Force is: ", self.actuatorForce_Max)
		print("Min Actuator Force is: ", self.actuatorForce_Min)
		with open("actuatorForce.txt", 'w') as f:
			for index, actuatorForce in enumerate(self.actuatorForces):
				f.write(str(index))
				f.write(": ")
				f.write(str(actuatorForce))
				f.write('\n')
