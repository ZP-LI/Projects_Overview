...
class ForeLegM(object):
	...
	def __init__(self, leg_params):
		...
		self.lr0 = leg_params['lr0'] # l_t0
		self.rp = leg_params['rp'] # r_p
		self.d1 = leg_params['d1'] # d
		...
		self.l_a = self.LawOfCosines_edge(
			self.l3, self.l4, (math.pi-self.alpha)) # AD
		self.beta = self.LawOfCosines_angle(
			self.l_a, self.l3, self.l4)

		self.PI = math.pi
	def pos_2_angle(self, x, y):
		lt = math.sqrt(x*x + y*y) # not l_t, but line|OD|
		theta_2_t = self.LawOfCosines_angle(self.l1, self.l_a, lt) # theta_t1
		theta_2 = theta_2_t + self.beta # theta_1
		#theta_1_t1 = self.LawOfCosines_angle(self.l1, lt, self.l_a)
		#theta_1_t2 = theta_1 - theta_1_t1

		theta_1_t1 = self.LawOfCosines_angle(self.l1, lt, self.l_a) # angle DOA
		theta_1_t2 = 0 # angle DO(-z)
		if x == 0:
			if y < 0:
				theta_1_t2 = 0
			else:
		...
		theta_1 = theta_1_t1 + theta_1_t2 # angle AO(-z), also q1
