import math
# Comments are based on Type 3 rear leg on the website:
# https://www.notion.so/Rear-Leg-Kinematics-391463559e4b4455a1df891a8a7283e8
class HindLegM(object):
	...
	def __init__(self, leg_params):
		...
		self.l_a = self.LawOfCosines_edge(
			self.l3, self.l4, (math.pi-self.alpha)) # |AH|
		self.beta = self.LawOfCosines_angle(
			self.l_a, self.l3, self.l4) # angle HAG

		self.PI = math.pi
	def pos_2_angle(self, x, y):
		lt = math.sqrt(x*x + y*y) # |CH|
		theta_2_t = self.LawOfCosines_angle(self.l1, self.l_a, lt) # angle CAH
		theta_2 = theta_2_t - self.beta # angle CAG
		#theta_1_t1 = self.LawOfCosines_angle(self.l1, lt, self.l_a)
		#theta_1_t2 = theta_1 - theta_1_t1

		theta_1_t1 = self.LawOfCosines_angle(self.l1, lt, self.l_a) # angle ACH
		theta_1_t2 = 0
		if x == 0:
			if y < 0:
				theta_1_t2 = 0
			...
			theta_1_t2 = math.acos(-y/lt)*(x/abs(x)) # angle HC(-z)
		theta_1 = theta_1_t1 - theta_1_t2 # angle AC(-z), also q1

		xB = self.l1*math.sin(theta_1) + self.l2*math.sin(theta_2-theta_1) # Problem: theta_1>0, theta_2>0, 为啥还是(theta_2-theta_1)?
